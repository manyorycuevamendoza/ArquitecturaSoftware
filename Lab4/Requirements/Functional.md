...
| FR-AI-16 | El sistema deberá distinguir la falta de conexión de la escuela de la indisponibilidad del proveedor de IA, y en ninguno de los dos casos perder la solicitud ni consumir la cuota reservada. | Un fallo del proveedor con red disponible (tiempo de espera agotado, error del servicio, límite de tasa o modelo retirado) deja la solicitud en espera con su cuota todavía reservada y su `requestId` intacto, distinguible en el registro de un `PENDING_NETWORK` por falta de red. El reintento es acotado: un número configurado de intentos con espera creciente, y al agotarse se devuelve el control al docente en lugar de seguir intentando. Una llamada que facturó tokens antes de fallar se registra con su consumo real; una que no llegó a facturar se registra con cero. Si el modelo de la plantilla dejó de estar disponible, la solicitud no se ejecuta contra otro modelo en silencio: se avisa a la coordinadora, porque cambiar de modelo altera la huella de caché y rompe la comparabilidad de la línea base. | Rosa, docente rural | PD-10, PD-12 |
...
| --- | --- | --- |
| **Rosa**, docente rural | Enseñar en la fecha en que toca, con o sin señal, y pedir material sin perder el tiempo ni gastar de más. | `FR-DIS-02`, `FR-DIS-03`, `FR-DIS-07`, `FR-DIS-10`, `FR-AI-01`, `FR-AI-03`, `FR-AI-11`, `FR-AI-12`, `FR-AI-13`, `FR-AI-16`, `NFR-NET-01`, `NFR-NET-03`, `NFR-INT-03`, `NFR-USA-01` |
...
| ¿Qué pasa si el proveedor de IA no responde? | `FR-AI-16`: no es lo mismo que no haya red; la solicitud espera con la cuota reservada y el reintento es acotado. |
| ¿Qué pasa si cambian de modelo? | `FR-AI-06`: el modelo entra en la huella, así que la caché no se contamina. `NFR-COST-03`: la línea base deja de ser comparable. `FR-AI-16`: no se sustituye el modelo en silencio. |
