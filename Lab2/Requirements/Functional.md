...
| FR-RSK-03 | Formal credit evaluation shall query an authorized negative-record source before any private credit-bureau score or payment-behavior query. | If the negative-record result is positive, the case moves to `CREDIT_REJECTED`, the bureau adapter is not called, and score, overdue-debt and late-payment rules are recorded as `NOT_EVALUATED`. | Credit-risk analyst |
| FR-RSK-04 | If the negative-record check is clear, the platform shall obtain an authorized credit-behavior report and apply a versioned credit policy. | The POC approves only when the simulated score is at least 650, overdue debt is S/0, late payments in the last 12 months are at most 2, and all required evidence is valid; every source, value and rule result is displayed. | Credit-risk analyst |
...
| ID | Requirement | Acceptance criterion | Responsible role |
| --- | --- | --- | --- |
| FR-DOC-01 | The platform shall maintain a checklist for the RUC record, signed customer project contract and recent bank statements, including each evidence state. | A case cannot be marked ready for the happy-path formal approval while any of the three evidence items is not valid. | Credit-risk analyst |
...
The React POC implements `FR-APP-01/02`, `FR-QUO-01/02`, `FR-RSK-01`, the manual-review decision branch of `FR-RSK-02`, deterministic simulated adapters and policies for `FR-RSK-03/04`, the valid-evidence and approval/rejection branches of `FR-DOC-01` and `FR-DEC-01`, `FR-OPS-01`, `FR-OPS-04`, and a visible in-memory case history that demonstrates part of `FR-AUD-01`.

The POC does not implement real document upload/validation, real negative-list/SBS/private-bureau access, request-for-information decisions, operations worklist filters (`FR-OPS-02`), accepted-term versioning (`FR-OPS-03`), authentication/authorization (`FR-ACC-01`) or an append-only production audit store (`FR-AUD-01`).
